...
class RestaurantArchitectureTest
{
    public function test_core_allowed_dependencies(): Rule
    {
        return PHPat::rule()
            ->classes(Selector::inNamespace('App\Restaurant\Core'))
            ->canOnlyDependOn()
            ->classes(
                ...array_merge(ArchitectureTest::allowedDomainDependencies(), [
                    Selector::inNamespace('App\Restaurant\Core'),
    ...
    {
        return PHPat::rule()
            ->classes(Selector::inNamespace('App\Restaurant'))
            ->shouldNotDependOn()
            ->classes(Selector::inNamespace('App'))
            ->excluding(
                Selector::inNamespace('App\Restaurant'),
